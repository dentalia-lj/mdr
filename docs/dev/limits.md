...
| `ImapEmailAdapter` | Built at `app/adapters/email.py:250` and already the default selection (`adapters.email = "imap"`, `app/config.py:200`; `email_poll.py:296` constructs it). What is missing is credentials, gap **G8**: with the `IMAP_*` keys empty `fetch_unseen` raises `EmailNotConfigured`, reported as an outcome rather than a failure, and `scheduler.email_poll_enabled` defaults `False` on top. No code change stands between the mailbox and this path |
