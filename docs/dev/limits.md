...
| BC OData ingest | `BcApiAdapter.read`, `app/adapters/source.py`, raises unless a client or records are injected | **Partly corrected 2026-09-14.** A real client IS built when `BC_BASE_URL` is set (`app/handlers/ingest.py`), so a CLI `enqueue ingest.run` whose `ref` is an absolute OData URL reaches the endpoint (the `BC_*` keys reach `worker` since 2026-09-24); with `BC_BASE_URL` empty it raises. Two things still block a cutover: (1) the `/ingest` FORM sends `ref` as `{company, delta_since}` and nothing turns that into a URL, so the browser option cannot work and dead-letters (`[ingest-bc-odata-always-fails]`); (2) no `$filter` is ever built, so `delta_since` is inert and every run is a full read. `LJ_ODATA_PROFILE`'s `manufacturer_raw` read the wrong property until 2026-09-25; it is `manufacturerCode`, measured on live BC that day. The endpoint is reachable only from the server's address (allowlist), over plain HTTP by ruling, with NTLM inside `Negotiate` (measured 2026-09-24). See [`../2026-09-03-bc-api-integration.md`](../2026-09-03-bc-api-integration.md) and [deployment.md](deployment.md) § 6.4 |
