...
4. **Business Central, read side.** The switch was always the planned second version, and `make_source_adapter` is a config choice over a closed enum. **The PascalCase profile and the missing-key guard were fixed 2026-09-07**: the profile maps camelCase and `_assert_profile_matches` raises `UnknownOdataProperty`. Two residues remain, both in [limits.md](docs/dev/limits.md) (a third, `manufacturer_raw` reading the wrong property, was **settled 2026-09-25** on live data: it is `manufacturerCode`): the `/ingest` form cannot build a `ref` URL, so the browser option always dead-letters (`[ingest-bc-odata-always-fails]`); no `$filter` is built, so `delta_since` is inert. **External access opened 2026-09-24**: `mail.dentalia.si:7048` from the server only, login NTLM inside `Negotiate` (built and verified live that day), `BC_*` wired to `worker`. The two residues are now provable against the real catalogue.
