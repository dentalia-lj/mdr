...
4. **Business Central, read side.** The switch was always the planned second version, and `make_source_adapter` is a config choice over a closed enum. **The PascalCase profile and the missing-key guard were fixed 2026-09-07**: the profile maps camelCase and `_assert_profile_matches` raises `UnknownOdataProperty`. Three residues remain, all in [limits.md](docs/dev/limits.md): `manufacturer_raw` reads `pteManufCodePrimary`, which is **present but empty** while `manufacturerCode` carries the value and the guard checks presence not emptiness (`[odata-manufacturer-property-disputed]`); the `/ingest` form cannot build a `ref` URL, so the browser option always dead-letters (`[ingest-bc-odata-always-fails]`); no `$filter` is built, so `delta_since` is inert. **External access opened 2026-09-24**: `mail.dentalia.si:7048` from the server only, login NTLM inside `Negotiate` (built and verified live that day), `BC_*` wired to `worker`. The three residues are now provable against the real catalogue.
5. **Business Central, write side. Complete as of 2026-09-07** — the rule, the `bc.push` stage, the ledger, the item button, the bulk preview-then-apply, the `bc.push-drift` cron. **Corrected and closed 2026-09-24:** the running handler was never given a BC client, sent the bare item number as the PATCH URL, and sent no `If-Match`; all three fixed that day, plus a separate `SCHEDULER_BC_PUSH_DRIFT_ENABLED` so the cron stays off after writes go on. Left: one supervised live PATCH, then the decision to set `bc.write_enabled` — F8. Writes stay off by Denis's decision. Design: `docs/superpowers/specs/2026-09-07-bc-writeback-design.md`.
